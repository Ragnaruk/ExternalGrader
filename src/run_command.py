...
def run_command(command: str,
    ...
    :param expected_program_name: string with a name of a program which should be called.
    :param path: string with a path where a program should run.
    :return: CompletedProcess which contains input arguments, returncode, stdout, and stderr.
    ...
    if len(expected_program_name) == 0:
        ...
        # Run program and return stdout and stderr in addition to arguments and returncode
        return subprocess.run([program_name, program_args], stdout=PIPE, stderr=PIPE, cwd=path)
